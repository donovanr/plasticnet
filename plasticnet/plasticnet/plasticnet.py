...
def _solve_ols_inplace(beta, r, X, thresh=1e-8, max_iters=100):
    r"""
    _solve_ols_inplace(beta, r, X, thresh=1e-8, max_iters=100)
...
def solve_ols(X, y, thresh=1e-8, max_iters=100):
    ...
    _solve_ols_inplace(beta, r, X, thresh=thresh, max_iters=max_iters)
...
def _solve_enet_inplace(
    beta, r, X, y, lambda_total=1.0, alpha=0.75, thresh=1e-8, max_iters=100
):
    r"""
    _solve_enet_inplace(beta, r, X, y, lambda_total=1.0, alpha=0.75, thresh=1e-8, max_iters=100)
    ...
    N, D = X.shape
    beta_old = beta.copy()
    delta_beta = np.ones(D, dtype=np.float64) + thresh
    rho = np.ones(D, dtype=np.float64) + thresh
