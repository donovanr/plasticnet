...
instructions at the [bottom of this page](#using-plastic_net-as-a-template).
...
`plastic_net/plastic_net.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `plastic_net.py` in directory called
`plastic_net`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import plastic_net as sb` in an
interactive Python session, the classes and functions defined inside of the
`plastic_net.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the plastic_net module, the main classes defined at the bottom of
...
Either way, you can create a `plastic_net/data` folder in which you can
...
(typically, something like our `plastic_net/tests/test_plastic_net.py`). Within each
...
have one such test in `plastic_net/tests/test_plastic_net.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/plastic_net`),
and type:
...
will see a message such as:
    ...
    plastic_net/tests/test_plastic_net.py .F...
    ...
    plastic_net/tests/test_plastic_net.py:49: AssertionError
...
To document `plastic_net` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/plastic_net/blob/master/doc/_static/RTD-advanced-conf.png)

 http://plastic_net.readthedocs.org/en/latest/
...
A `plastic_net/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/plastic_net) for the software. This
...
`plastic_net` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `plastic_net`, we use the
...
[here](https://github.com/uwescience/plastic_net/blob/master/scripts/Figure1.ipynb).
...
### Using `plastic_net` as a template
...
Under "old repository clone URL" enter: https://github.com/uwescience/plastic_net
...
`plastic_net/plastic_net.py` to be called `smallish/smallish.py`

	git mv plastic_net smallish
	git mv smallish/plastic_net.py smallish/smallish.py
	git mv smallish/tests/test_plastic_net.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `plastic_net` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `plastic_net` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `plastic_net` in them. Type
the following to see where all these files are:

	git grep plastic_net

You can replace `plastic_net` for `smallish` quickly with:

	git grep -l 'plastic_net' | xargs sed -i 's/plastic_net/smallish/g'
...
Edit all the mentions of `plastic_net` in the `plastic_net/__init__.py` file, and in
the `plastic_net/version.py` file as well.
